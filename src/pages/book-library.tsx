...
export default function BookLibrary() {
  ...
  const previewBook = async (book: BookPreview) => {
    ...
    try {
      ...
      const response = await fetch('/api/books/generate-book', {
        ...
        body: JSON.stringify({ theme: book.theme }),
      });

      if (response.ok) {
        const data = await response.json();
        
        // Import puzzle name generators
        ...
        // Get theme data for unique names
        const themeData = getExpandedThemeDataComplete(book.theme);
        const wordSearchNames = generateWordSearchNames(book.theme);
        const crosswordNames = generateCrosswordNames(book.theme);
        
        // Create full puzzle previews - 75 word searches + 75 crosswords = 150 total
        const puzzles: PuzzlePreview[] = [];
        const difficulties = ['Easy', 'Medium', 'Hard'];
        
        // Generate 75 word search previews with unique names
        for (let i = 1; i <= 75; i++) {
          puzzles.push({
            id: `ws_${i}`,
            type: 'wordsearch',
            title: wordSearchNames[i - 1] || `${book.theme} Word Search ${i}`,
            difficulty: difficulties[(i - 1) % 3],
            words: themeData?.words.slice(0, 10) || [`SAMPLE`, `WORD`, `LIST`, `FOR`, `PREVIEW`]
          });
        }
        
        // Generate 75 crossword previews with unique names
        for (let i = 1; i <= 75; i++) {
          puzzles.push({
            id: `cw_${i}`,
            type: 'crossword',
            title: crosswordNames[i - 1] || `${book.theme} Crossword ${i}`,
            difficulty: difficulties[(i - 1) % 3],
            clues: themeData?.crosswordData.slice(0, 5).map((item, idx) => ({
              number: idx + 1,
              clue: item.clue,
              answer: item.word,
              direction: (idx % 2 === 0 ? 'across' : 'down') as 'across' | 'down'
            })) || [
              { number: 1, clue: 'Sample clue', answer: 'ANSWER', direction: 'across' },
              { number: 2, clue: 'Another clue', answer: 'WORD', direction: 'down' }
            ]
          });
        }
  ...
  const downloadPDF = async (book: BookPreview) => {
    setGenerating(true);
    try {
      // Generate the actual book with all puzzles
      const response = await fetch('/api/books/generate-book', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        // Import the generators, theme data, and puzzle utilities
        ...
        const { getExpandedThemeDataComplete } = await import('@/utils/elderlyFriendlyThemes');
        const { generateWordSearchNames, generateCrosswordNames, generateUniqueWordSets, generateUniqueCluesets } = await import('@/utils/puzzleNameGenerator');
        
        // Get theme data
        const themeData = getExpandedThemeDataComplete(book.theme);
        if (!themeData) {
          toast.error('Theme data not found');
          return;
        }

        // Generate unique puzzle names
        const wordSearchNames = generateWordSearchNames(book.theme);
        const crosswordNames = generateCrosswordNames(book.theme);
        
        // Generate unique word sets and clue sets to ensure no repetition
        const uniqueWordSets = generateUniqueWordSets(themeData.words, 75, 12); // 75 puzzles, max 12 words each
        const uniqueClueSets = generateUniqueCluesets(themeData.crosswordData, 75, 10); // 75 puzzles, max 10 clues each

        // Create LARGE PRINT PDF optimized for seniors
        const pdf = new jsPDF('p', 'mm', 'a4');
        const pageWidth = pdf.internal.pageSize.getWidth();
        const pageHeight = pdf.internal.pageSize.getHeight();
        
        // SENIOR-FRIENDLY MARGINS: Generous margins for easy handling and binding
        const margin = 25; // Increased from 15mm to 25mm for better grip space
        const contentWidth = pageWidth - (2 * margin);
        const contentHeight = pageHeight - (2 * margin);
        let pageNumber = 1;

        // Helper function to add LARGE PRINT page header
        const addPageHeader = (title: string, pageNum: number) => {
          pdf.setFontSize(14); // Increased from 10 to 14 for better visibility
        ...
        // Helper function to add LARGE PRINT page footer
        const addPageFooter = (bookTitle: string, pageNum: number) => {
          pdf.setFontSize(12); // Increased from 8 to 12
          pdf.setFont('helvetica', 'normal');
          pdf.setTextColor(0, 0, 0);
          pdf.text(bookTitle, pageWidth / 2, pageHeight - 10, { align: 'center' });
        };

        // TITLE PAGE - LARGE PRINT DESIGN
        pdf.setFontSize(36); // Increased from 28 to 36 for better readability
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(0, 0, 0);
        ...
        pdf.setFont('helvetica', 'normal');
        pdf.text(`Theme: ${book.theme}`, pageWidth / 2, 85, { align: 'center' });
        
        // Description with larger font and better spacing
        pdf.setFontSize(16); // Increased from 12 to 16
        const descLines = pdf.splitTextToSize(book.description, contentWidth - 20);
        pdf.text(descLines, pageWidth / 2, 110, { align: 'center' });
        
        // Book stats with LARGE PRINT
        pdf.setFontSize(20); // Increased from 14 to 20
        pdf.setFont('helvetica', 'bold');
        pdf.text('Book Contents:', pageWidth / 2, 150, { align: 'center' });
        
        pdf.setFontSize(16); // Increased from 12 to 16
        pdf.setFont('helvetica', 'normal');
        pdf.text(`• ${book.wordSearchCount} Word Search Puzzles`, pageWidth / 2, 170, { align: 'center' });
        pdf.text(`• ${book.crosswordCount} Crossword Puzzles`, pageWidth / 2, 185, { align: 'center' });
        pdf.text(`• ${book.totalPages} Total Pages`, pageWidth / 2, 200, { align: 'center' });
        pdf.text('• Complete Answer Keys Included', pageWidth / 2, 215, { align: 'center' });

        // Senior-friendly instructions
        pdf.setFontSize(14);
        pdf.setFont('helvetica', 'bold');
        pdf.text('LARGE PRINT EDITION', pageWidth / 2, 240, { align: 'center' });
        pdf.setFont('helvetica', 'normal');
        pdf.text('Designed for Easy Reading and Comfortable Solving', pageWidth / 2, 255, { align: 'center' });

        addPageFooter(book.title, pageNumber++);

        // Save the PDF with descriptive filename
        const filename = `${book.title.replace(/\s+/g, '_')}_LARGE_PRINT_Senior_Friendly.pdf`;
        pdf.save(filename);
        toast.success('Large Print Senior-Friendly PDF downloaded successfully!');
      } else {
